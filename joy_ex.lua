...
local joyex__guicai = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    if not player.dead and self.cost_data.suit == Card.Club  then
      player:drawCards(2, self.name)
    end
    if not player.dead and self.cost_data.suit == Card.Heart  then
      player.room:recover({
      who = player,
      num = 1,
      recoverBy = player,
...
local joyex__fankui = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    if judge.card.suit == Card.Heart and from and not from.dead then
      ...
      if #targets == 0 then return end
      local tos = room:askForChoosePlayers(player, targets, 1, 1, "#joyex__fankui-discard", self.name, false)
      local to = room:getPlayerById(tos[1])
      local card1 = room:askForCardChosen(player, to, "he", self.name)
      room:obtainCard(player, card1, false, fk.ReasonPrey)
    elseif judge.card.suit ~= Card.Heart and from and not from:isNude() then
      local card2 = room:askForCardChosen(player, data.from, "he", self.name)
...
Fk:loadTranslationTable{
  ...
  ["#joyex__fankui-discard"] = "反馈：选择任意一名角色获得其一张牌。",
